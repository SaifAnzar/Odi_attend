          ...
          <button
            ...
            className={`pb-3 text-sm font-semibold border-b-2 transition-all cursor-pointer ${
              employeeTab === 'incoming'
                ...
                : 'border-transparent text-odizo-grey hover:text-slate-900 dark:text-white dark:hover:text-white'
            }`}
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {incomingSwaps.map((swap) => (
                  <RequestCard
                    ...
                    id={swap._id}
                    type="Swap"
                    employeeName={swap.requesterId?.name || 'Unknown'}
                    employeeRole={swap.requesterId?.role || 'Employee'}
                    employeeEmail={swap.requesterId?.email}
                    startDate={swap.swapDate}
                    status={swap.status === 'Pending Target' ? 'Waiting on You' : swap.status}
                    reason={`Shift Swap requested by colleague`}
                    details={`Colleague Shift: ${swap.requesterId?.shift?.name || 'Standard'} (${swap.requesterId?.shift?.startTime}-${swap.requesterId?.shift?.endTime}) ➔ Your Shift: ${swap.targetUserId?.shift?.name || 'Standard'} (${swap.targetUserId?.shift?.startTime}-${swap.targetUserId?.shift?.endTime})`}
                    showActions={true}
                    actionLoading={actionLoading}
                    onApprove={() => handleReviewSwap(swap._id, 'Pending Admin')}
                    onReject={() => handleReviewSwap(swap._id, 'Rejected')}
                  />
                ))}
              </div>
            )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {outgoingSwaps.map((swap) => (
                  <RequestCard
                    ...
                    id={swap._id}
                    type="Swap"
                    employeeName={swap.targetUserId?.name || 'Unknown'}
                    employeeRole={swap.targetUserId?.role || 'Employee'}
                    employeeEmail={swap.targetUserId?.email}
                    startDate={swap.swapDate}
                    status={swap.status === 'Pending Target' ? 'Waiting on Colleague' : swap.status}
                    reason={`Shift Swap requested with colleague`}
                    details={`Your Shift: ${swap.requesterId?.shift?.name || 'Standard'} (${swap.requesterId?.shift?.startTime}-${swap.requesterId?.shift?.endTime}) ➔ Colleague Shift: ${swap.targetUserId?.shift?.name || 'Standard'} (${swap.targetUserId?.shift?.startTime}-${swap.targetUserId?.shift?.endTime})`}
                    showActions={false}
                  />
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(activeTab === 'pending' ? swaps.filter(s => s.status === 'Pending Admin') : swaps).map((swap) => (
              <RequestCard
                ...
                startDate={swap.swapDate}
                status={swap.status}
                reason={`Shift Swap with ${swap.targetUserId?.name || 'colleague'}`}
                details={`Requester Shift: ${swap.requesterId?.shift?.name || 'Standard'} (${swap.requesterId?.shift?.startTime}-${swap.requesterId?.shift?.endTime}) ➔ Colleague Shift: ${swap.targetUserId?.shift?.name || 'Standard'} (${swap.targetUserId?.shift?.startTime}-${swap.targetUserId?.shift?.endTime})`}
                showActions={activeTab === 'pending'}
                actionLoading={actionLoading}
                onApprove={() => handleApprove(swap._id)}
                onReject={() => handleRejectClick(swap._id)}
              />
            ))}
          </div>
        )}
